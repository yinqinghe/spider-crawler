...
from openpyxl import load_workbook  # 用于读取已有的工作簿
import pandas as pd
...
# writer = pd.ExcelWriter(r'data.xlsx', engine='openpyxl')
# save_data=load_workbook(savepath)
# writer.book=save_data
tem_data = xlrd.open_workbook(savepath, formatting_info=True)
tem_wb = copy(wb=tem_data)
tem_sheet=tem_wb.add_sheet('eee')

wb = Workbook()
ws = wb.create_sheet('data2', 3)
f1 = open('D:\Pics\log.txt', mode="a", encoding='utf-8')
    ...
    for item in items:
        ...
        try:
            ...
            for jj in range(0, len(message)):  # 写入数组信息
                tem_sheet.write(j, jj, message[jj])
                # sheet.to_excel(writer, sheet_name='1')
                # ws['%s%d' % (chr(ASC_i),j)]=message[jj]
                # ASC_i+=1
